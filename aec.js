...
(function(){
  var request, iso8601, cheerio, stations, radiations, out$ = typeof exports != 'undefined' && exports || this;
  ...
  radiations = function(respond){
    ...
    return request({
      url: 'http://www.trmc.aec.gov.tw/utf8/big5/main.php',
      encoding: "utf-8"
    }, function(_err, _res, page){
      var radiations, $;
      if (_err) {
        console.log("aec error: " + JSON.stringify(_err));
      }
      radiations = [];
      $ = cheerio.load(page);
      $("table a.a").each(function(){
        if (this.attr(/^javascript/.exec('href'))) {
          return radiations.push({
            location: trim(this.text()),
            time: new Date(),
            value: trim(this.parent().next().next().text())
          });
        }
      });
      return respond(radiations);
    });
  };
